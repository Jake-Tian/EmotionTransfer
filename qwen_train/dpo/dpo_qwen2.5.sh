...
MODEL_PATH="${PROJECT_DIR}/models/Qwen2.5-0.5B-Instruct-sft"  # Use your SFT checkpoint
OUTPUT_DIR="${PROJECT_DIR}/models/Qwen2.5-0.5B-Instruct-dpo"
...
torchrun --standalone --nnodes=1 --nproc-per-node=${NUM_GPUS} src/train.py \
    ...
    --eval_strategy no \
    > ${PROJECT_DIR}/qwen_train/dpo/train_log.txt 2>&1
